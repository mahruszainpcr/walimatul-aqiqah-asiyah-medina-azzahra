...
const supabaseKey = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im9zdm55c296amp6emtrZXZobGZtIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTAxMzA4NjMsImV4cCI6MjA2NTcwNjg2M30.sEPSdAu6PQ0lQZQRYPfxwnp2VpSLVmwl9kJ_IuADj5o';
...
function DoaHarapanInteraktif() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const { error } = await supabase.from('doa_harapan').insert([
      { nama, doa },
    ]);
    if (!error) {
      setSubmitted(true);
    ...
    <section className="bg-[#090607] text-white font-montserrat px-6 py-10 min-h-screen flex flex-col items-center justify-center text-center">
      <div className="max-w-xl w-full space-y-6">
        ...
        <p className="text-sm text-gray-300 italic">
          Kirimkan doa terbaik Anda untuk Aisyah Medina Azzahra. Setiap kata adalah berkah yang berarti bagi keluarga kami.
        </p>

        {/* Form */}
        <div className="bg-[#221f20] p-6 rounded-lg shadow space-y-4">
          {submitted ? (
            <p className="text-green-400 text-sm">Terima kasih atas doa dan harapan yang telah Anda kirimkan 💖</p>
          ) : (
        ...
        <div className="mt-8 space-y-4">
          <h3 className="text-lg font-semibold text-[#c26e73]">💌 Doa dari Tamu</h3>
          {listDoa.length === 0 ? (
            <p className="text-sm text-gray-400 italic">Belum ada doa yang masuk.</p>
          ) : (
            listDoa.map((item) => (
              <div key={item.id} className="bg-[#221f20] p-4 rounded shadow text-left">
